        ...
        if "ID8471214" in theJSON["gameData"]["players"]:
            ...
            if (theJSON["gameData"]["venue"]["name"] in homeVenues):
                if not theJSON["liveData"]["boxscore"]["teams"]["home"]["players"]["ID8471214"]["stats"]:
                    ...
                    plays = timeScrape(gameID, "home", theJSON)
                    return plays
            else:
                if not theJSON["liveData"]["boxscore"]["teams"]["away"]["players"]["ID8471214"]["stats"]:
                    print("Ovechkin rostered but did not play")
                else:
                    plays = timeScrape(gameID, "away", theJSON)
                    return plays
        else:
...
def main():
    ...
    print(finalJSON)
    scheduleUrl = str("https://statsapi.web.nhl.com/api/v1/schedule?teamId=15&season=" + str(currentseason) + str(currentseason + 1) + "&gameType=R")
    print(scheduleUrl)
